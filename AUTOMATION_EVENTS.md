...
|-------|------|-------------|
| `entity_id` | string or null | **DEPRECATED - Will be removed in a future version.** The climate entity that was updated (e.g., `climate.bedroom`), or `null` for virtual schedules. For single-entity events, this is the specific entity. For backwards compatibility only. |
| `entities` | list | All climate entities in the group (e.g., `["climate.bedroom"]`). Empty list for virtual schedules. **Use this field for all automations.** |
